...
func (oq *OrganizationQuery) sqlAll(ctx context.Context) ([]*Organization, error) {
	...
	if query := oq.withUsers; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(organization.UsersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "organization_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "organization_users" returned %v for node %v`, *fk, n.ID)
			}
