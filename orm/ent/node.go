...
// Node in the graph.
type Node struct {
	ID     qrn.ID   `json:"id,omitempty"`     // node id.
...
type Edge struct {
	...
	IDs  []qrn.ID `json:"ids,omitempty"`  // node ids (where this edge point to).
}
...
func (c *Client) Node(ctx context.Context, id qrn.ID) (*Node, error) {
...
func WithNodeType(f func(context.Context, qrn.ID) (string, error)) NodeOption {
...
func WithFixedNodeType(t string) NodeOption {
	return WithNodeType(func(context.Context, qrn.ID) (string, error) {
		return t, nil
	})
}

type nodeOptions struct {
	nodeType func(context.Context, qrn.ID) (string, error)
}
...
func (c *Client) newNodeOpts(opts []NodeOption) *nodeOptions {
	...
	if nopts.nodeType == nil {
		nopts.nodeType = func(ctx context.Context, id qrn.ID) (string, error) {
...
//
func (c *Client) Noder(ctx context.Context, id qrn.ID, opts ...NodeOption) (_ Noder, err error) {
	defer func() {
		if IsNotFound(err) {
...
func (c *Client) noder(ctx context.Context, table string, id qrn.ID) (Noder, error) {
	switch table {
...
func (c *Client) Noders(ctx context.Context, ids []qrn.ID, opts ...NodeOption) ([]Noder, error) {
	switch len(ids) {
	case 1:
	...
	tables := make(map[string][]qrn.ID)
	id2idx := make(map[qrn.ID][]int, len(ids))
...
func (c *Client) noders(ctx context.Context, table string, ids []qrn.ID) ([]Noder, error) {
	noders := make([]Noder, len(ids))
	idmap := make(map[qrn.ID][]*Noder, len(ids))
	for i, id := range ids {
